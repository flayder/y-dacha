...
import {loadDisease} from "../store/actions/diseases";
import {FAVORITE_DISEASE, getRandomKey, globalRouteFun} from "../../global";
...
const pageWrapper = ({navigation, route}) => {
    ...
    if(route.params !== undefined) {
        if(route.params.hasOwnProperty('diseaseId')) {
            id = route.params.diseaseId
        }
    }
    if(id === 0) {
       return (
    ...
    useEffect(() => {
        dispatch(loadDisease({id, navigation}))
    }, [init])

    const data = useSelector(state => state.diseases.currentDisease)
    ...
    return (
        <AppWrap scroll={{paddingLeft: 0, paddingRight: 0}}>
            <View style={{paddingLeft: 20, paddingRight: 20}}>
                <AppImageWrapper title={data.name} style={styles.img}>
                    ...
                    <AppFavoriteButton
                        ...
                        hintText={"Заболевание было успешно добавлено в избранное"}
                        hintReserved={true}
                        activeForm={() => {
            ...
            <AppAddPhotoButton routeName="DiseasesAddPhotoPageScreen" navigation={navigation} />
