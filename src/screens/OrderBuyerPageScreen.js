...
import AppDeliveryDateBlock from "../ui/orderBuyerPageScreen/AppDeliveryDateBlock";
import {Path, Rect, Svg} from "react-native-svg";
import AppRadio from "../ui/formUI/AppRadio";
import {AppText} from "../ui/AppText";
import AppPayable from "../ui/AppPayable";
...
const pageWrapper = ({navigation, route}) => {
    ...
    const initiate = async () => {
        const response = await AppFetch.getWithToken("getSellerOrders", {}, {}, navigation)
    ...
    const parseDelivery = (del, globalKey) => {
        ...
        deliveries.current.map(item => {
            for(let code in del) {
                if(del[code] && item.hasOwnProperty('code') && item.code == code) {
                    elements.push(
                        <TouchableOpacity
                            activeOpacity={1}
                            ...
                            onPress={() => {
                                ...
                                if (formData[globalKey] != undefined) {
                                    ...
                                    deliveries.current.map(item1 => {
                                        if (item1.code == code) id = item1.id
                                    })
                                    if (id) {
                        ...
                        >
                            <AppRadio
                                activeIs={formData[globalKey] != undefined && formData[globalKey].delivery_method_id > 0 && getDeliveryId(code) == formData[globalKey].delivery_method_id}
                                onResult={() => {
                                    let id = false
                                    deliveries.current.map(item1 => {
                                        if (item1.code == code) id = item1.id
                            ...
                            />
                            <AppText
                                style={styles.radioBlockText}
                            >
                                {del[code]}
                                {
                                    del.hasOwnProperty(code + "_price") && parseInt(del[code + "_price"]) > 0
                                        ?
                                        " (+" + parseInt(del[code + "_price"]) + "₽)"
                                        :
                                        <></>
                                }
                            </AppText>
        ...
        if(Array.isArray(address)) {
            address.map((item, key) => {
                ...
                elements.push(
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            if(formData[globalKey] != undefined) {
                                ...
                                //console.log('formData', formData[globalKey], item)
                                setData(formData)
                                setReload(!reload)
                            }
                        }}
                    >
                        <AppRadio
                            activeIs={formData[globalKey] != undefined && formData[globalKey].address == item}
                            onResult={() => {
                                formData[globalKey].address = item
                                //console.log('formData', formData[globalKey], item)
                                setData(formData)
                    ...
                    Array.isArray(data) && data.length > 0
                        ...
                        data.map((item, globalKey) => {
                            ...
                            if(item.delivery_method_id > 0
                                ...
                                const delCode = getDeliveryCode(item.delivery_method_id)
                                if(delCode) {
                                    if(item.owner.infoShop.delivery_methods.hasOwnProperty(delCode + "_price") && item.owner.infoShop.delivery_methods[delCode + "_price"]) {
                                        const pr = parseInt(item.owner.infoShop.delivery_methods[delCode + "_price"])
                                        if(pr > 0) total += pr
                                    }
                                }
                            }
