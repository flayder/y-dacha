...
import {loadAllShops, setShopsPagination} from "../store/actions/shops";
...
import AppAllShopsItem from "../ui/shopsAllPageScreen/AppAllShopsItem";
...
const pageWrapper = ({navigation, route}) => {
    ...
    useEffect(() => {
        if(shopInitiation.current) {
            dispatch(loadAllShops({loadMore: false, navigation})).then(res => {
                //console.log('res', res)
            })
        }
    ...
    //loadBydefault
    useEffect(() => {
        dispatch(loadAllShops({type, navigation})).then(res => {
            //setData(res)
        })
        //setData(true)
    }, [pagination])
