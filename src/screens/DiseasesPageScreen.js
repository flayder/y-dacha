...
import {loadFilter, setFilterType, emptyFilter} from "../store/actions/filter";
...
import {loadDiseases, setDiseasesPagination} from "../store/actions/diseases";
...
const pageWrapper = ({navigation, route}) => {
    ...
    const itemsState = useSelector(state => state.diseases)
    ...
    const counts = useSelector(state => state.diseases.counts)
    ...
    useEffect(() => {
        ...
        dispatch(loadFilter("getDiseasesFilter", {
            type,
            id
        }, navigation))
        if(pagination > 1) dispatch(setDiseasesPagination(1))
    ...
    useEffect(() => {
        if(!loadMore) {
            dispatch(setDiseasesPagination(pagination + 1)).then(res => {
                setTimeout(() => {
                    setLoadMore(true)
                    //console.log('done')
                }, 500)
    ...
    //loadBydefault
    useEffect(() => {
        dispatch(loadDiseases({id, type, navigation})).then(res => {
           //setData(res)
            //console.log('here')
        })
        //setData(true)
    ...
    useEffect(() => {
        ...
        if(filterInitiation.current)
            dispatch(loadDiseases({id, type, loadMore: false, navigation})).then(res => {
               //setData(res)
               //console.log('ok')
            })
        else
    ...
    return (
        <>
            ...
            <AppWrap scroll={{paddingLeft: 0, paddingRight: 0}} onScroll={scrollerHandler}>
                ...
                <AppCultureTitle title={'Заболевания культуры "' + preview.name + '"'} />
                <AppAddPhotoButton style={{marginTop: -20}} routeName="DiseasesAddPhotoPageScreen" navigation={navigation} />
                <View style={styles.container}>
                    {
                        data.map(item => {
                            return <AppPestsItem
                                color={color}
                                item={item}
                                routeName="DiseaseDetailPageScreen"
                                id="diseaseId"
