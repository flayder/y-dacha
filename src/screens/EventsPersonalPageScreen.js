...
import {loadEvents, loadUserEvents} from "../store/actions/events";
...
const pageWrapper = ({navigation, route}) => {
    ...
    //console.log('date', regions)

    useEffect(() => {
        if(!init.current)
            dispatch(loadUserEvents({navigation}))
        else {
            init.current = true
            dispatch(loadUserEvents({navigation, init: false}))
        }
    }, [date, selected])
            ...
            <AppWrap scroll={{paddingLeft: 0, paddingRight: 0}} onScroll={scrollerHandler}>
                ...
                <View style={styles.selecter}>
                    ...
                    <AppSelectRegions
                        ...
                        onResult={region => {
                            if(region > 0)
                                dispatch(loadUserEvents({navigation, region}))
                            else
                                dispatch(loadUserEvents({navigation}))
                        }}
                    />
                ...
                <View style={styles.container}>
                    {
                        (Array.isArray(data) && data.length > 0)
                            ?
                            <View style={{width: "100%", minHeight: data.length * 140}}>
                                {
                                    data.map((item, key) => {
                                        return <AppEventItem
                                            editable={true}
                                            ...
                                            screenWidth={screeWidth + 10}
                                            onDelete={async () => {
                                                await AppFetch.getWithToken('deleteEvent', {id: item.id}, {}, navigation)
                                                dispatch(loadUserEvents({navigation}))
                                            }}
                                        />
                                    })
                    ...
                    <AppButton
                        ...
                        style={{width: 250, marginTop: 20, marginBottom: 80, marginRight: "auto", marginLeft: "auto"}}
                    >
