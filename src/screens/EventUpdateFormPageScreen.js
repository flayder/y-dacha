...
const pageWrapper = ({navigation, route}) => {
    ...
    useEffect(() => {
        AppFetch.getWithToken('getEventDetail',
            {id},
            {},
            navigation
        ).then(result => {
            ...
            if(data.hasOwnProperty('days')) {
                for(let d in data.days) {
                    days.push(data.days[d])
                }
                formData.current.days = days
            }

            if(data.hasOwnProperty('event')) {
                formData.current.title = data.event.title
                formData.current.date = data.event.date
                formData.current.place = data.event.location
                formData.current.type = data.event.type
                formData.current.price = data.event.price
                formData.current.email = data.event.contact_email
                formData.current.phone = data.event.contacts
                formData.current.region = data.event.region
                formData.current.photos = []
                const photos = []
                photos.push(makeFileObject(data.event.main_photo))

                if(data.event.hasOwnProperty('photos') && Array.isArray(data.event.photos) && data.event.photos.length > 0) {
                    data.event.photos.map(path => {
                        photos.push(makeFileObject(path))
                    })
                }
                formData.current.photos = photos
                //console.log('photos', formData.current.photos)
            }
    ...
    //console.log('okokok', formData.current)
    ...
    const sendToModeration = async () => {
        ...
        //
        //                 // if(day.date != "") {
        //                 //     if(day.time == "" || day.description == "") error = true
        //                 // }
        //
        //                 if(day.description != "") {
        ...
        if(!error) {
            ...
            data.append('countFiles', formData.current.photos.length)
            //console.log('formData.current', JSON.stringify(data))
            //return
            const response = await AppFetch.postWithToken('updateEvent', data, {}, {
            ...
            if(response.result) {
                globalAlert({
                    title: "Событие было успешно отредактированно!",
                    onOkFun() {
